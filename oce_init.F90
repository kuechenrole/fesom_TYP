!1) allocate ocean arrays
!2) initialize ocean fields
...
subroutine ocean_init
     ...
     if (case_initial=='warm') then
             ...
             do n=1,myDim_nod2d+eDim_nod2d
                 do k=1,num_layers_below_nod2d(n)+1
                    ...
                    !tracer(n3,1)=1.0 + (-1.9-1.8)/720.*(z+720.0)
                    !tracer(n3,2)=34.7 + (33.8-33.9)/720.*(z+720.0)
                 end do
             end do
 ...
 ! store initial surface TS values (e.g. for SSS restoring)
   do i=1, ToDim_nod2D     
     node=nod3D_below_nod2D(1,i)       
     Tsurf(i)=tracer(node,1)          
     Ssurf(i)=tracer(node,2)          
   end do
 ! RT: note that this block used to sit below the r_restart block that follows. 
 ! RT: I just do not see why that makes sense, so I moved the block here.
 ! RT RG45970, 20.08.2019 
  ...
  if(r_restart) then
      ...
      if (use_passive_tracer) then 
       ...
       call passive_tracer_input     !RT: Switch this off if you want to reinitialize passive tracers
      end if                     
...
subroutine set_coriolis_param
  ! Setup coriolis parameters, both (2D) nodal and elementwise.
  !
  ! Coded by Qiang Wang
  ! Reviewed by ??
  !-------------------------------------------------------
  ...
  coriolis_param_nod2D=2.0*omega*sin(geolat2d)

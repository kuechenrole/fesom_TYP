...
subroutine init_cavity_ts_extrapolate
  ...
  integer                     :: i, j, n, n2, n_bd, row, ind, nk
  ...
  if (mype .eq. 0) write(*,*) 'in init_cavity_ts_extrapolate'
  ...
  !extrapolate to the nodes under cavity
  do row=1, myDim_nod3d+eDim_nod3D  
     ...
     write(mype+1200,'(i6,10f7.2,i6)')row,tracer(row,1),tracer(row,2),pr,pp,aux1,aux2,coord_nod3d(1,row),coord_nod3d(2,row),coord_nod3d(3,row),dep_op_glo(n_bd),n_bd
  end do
...
!       write(2200+mype,'(3f7.2,2i8)') tracer(nk,1),tracer(nk,2),coord_nod3d(3,nk),n,nk
!    enddo
...
!!RT test-
     ...
     if (variable_GammaTS==.true.) then
       ...
       gat = GammaT*sak1*vt1
       gas = gat/35.0
       !write(*,'(a10,9f10.3)') 'GammaTS = ',gat,gas
     endif

     !RG3417 gat  = 1.00e-4   ![m/s]  old, constant values
     !RG3417 gas  = 5.05e-7   ![m/s]  old, constant values



     ! Calculate density in the boundary layer: rhow
     call fcn_density(temp,sal,zice,rho)
     rhow = rho !fcn_density returns full in-situ density now!
     ! was rhow= rho0+rho(i,j,N) in BRIOS


     ! Solve a quadratic equation for the interface salinity sf
     rhor= rhoi/rhow
     ep1 = cpw*gat
     ep2 = cpi*gas
     ep3 = lhf*gas
     ep31 = -rhor*cpi*tdif/min(zice,-100.)   ! RG47908: added min(zice,-100.)
     ep4 = b+c*zice
     ep5 = gas/rhor
...
subroutine potit(salz,pt,pres,rfpres,tin,x,y)
  ...
  enddo
;  write(6,*) ' WARNING!'
  write(6,*) ' in-situ temperature calculation has not converged:',x,y,salz,pt,pres,rfpres,tin,pt1,ptd
  stop
  return
